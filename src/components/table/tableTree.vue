            ...
            <tbody>
                <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        ...
                        <div v-if="column.type === 'action'" class="action-col">
                            <i-button
                        ...
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                ...
                                <i v-if="item[childrenKey]&&item[childrenKey].length>0" class="ivu-icon" :class="{'ivu-icon-plus-round':!item.expanded,'ivu-icon-minus-round':item.expanded }"></i>
      ...
      iconRow() {
        ...
        initData(items, level, parent) {
            ...
            items.forEach((item, index) => {
                ...
                if (item[this.childrenKey] && item.expanded) {
                    this.initData(item[this.childrenKey], level + 1, item);
                }
            })
        },
        //  隐藏显示
            ...
            if (item[this.childrenKey]) {
                if (item.expanded) {
                    ...
                    if (item.load) {
                        ...
                        item.load = true;
                        item[this.childrenKey].forEach((child, childIndex) => {
        ...
        open(index, item) {
            if (item[this.childrenKey]) {
                item[this.childrenKey].forEach((child, childIndex) => {
                    child.isShow = true;
                    if (child[this.childrenKey] && child.expanded) {
        ...
        All(data) {
            ...
            data.forEach((item) => {
                ...
                if (item[this.childrenKey] && item[this.childrenKey].length > 0) {
                    arr = arr.concat(this.All(item[this.childrenKey]));
                }
            })
            return arr
        },
        ...
        Length(data) {
            ...
            data.forEach((child) => {
                if (child[this.childrenKey]) {
                    length += this.Length(child[this.childrenKey])
                }
            })
            return length;
        },
        ...
        renderCheck(data) {
            ...
            data.forEach((item) => {
                ...
                if (item[this.childrenKey] && item[this.childrenKey].length > 0) {
                    arr = arr.concat(this.renderCheck(item[this.childrenKey]));
                }
            })
            return arr
        },

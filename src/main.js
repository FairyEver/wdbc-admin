...
import {routers, otherRouter, appRouter} from '@/router/index.js';
...
// prism代码高亮
import '@/assets/library/prism/prism.css';
...
Vue.directive('p', {
    bind: (el, binding) => {
        if (vm.$store.state.permissionList.find(e => e === binding.value) === undefined) {
...
const store = new Vuex.Store({
    ...
    mutations: {
        ...
        setPermissionList (state, list) {
            ...
            localStorage.permissionList = JSON.stringify(state.permissionList);
        },
        reloadPermissionList (state) {
            state.permissionList = localStorage.permissionList ? JSON.parse(localStorage.permissionList) : [];
        },
        // 左侧菜单列表
...
const vm = new Vue({
    ...
    methods: {
        p (name) {
            // 权限
            return !!(this.$store.state.permissionList.find(e => e === name));
        }
    }
});

...
<template>
    <div>
        <LayoutHeader title="性状记录"></LayoutHeader>
...
export default {
        ...
        return {
            ...
            tableGT: {
                columns: [
                    ...
                    {title: '日期', key: 'dateOfMeasurement'},
                    {title: '日龄', key: 'days', width: 70},
                    {title: '周龄', key: 'week', width: 70},
                    {title: '性状指标', key: 'traitIndex'},
                    {title: '值', key: 'value', width: 70},
                    {title: '单位', key: 'company', width: 70}
                ],
                data: [],
                deleteMode: true
            },
    ...
    methods: {
        ...
        syncGT () {
            ...
            axiosToken({
                url: '/a/chicken/chickenIndividualCharacter',
                data: {
                ...
                let rowMaker = ({
                    ...
                    wb,
                    identityNumber,
                    dateOfMeasurement: moment(dateOfMeasurement).format("YYYY-MM-D"),
                    days,
                    week,
                    traitIndex,
                    company
                ...
                // 表格数据 只要需要提交的
                this.tableGT.data = page.list ? page.list.map(e => ({
                    ...rowMaker(e),
                    value: e.fieldType === 1 ? e.dubValue : e.value
                })) : [];
                // 无论是个体还是群体都会用到的设置
                this.syncPub(res);
        ...
        syncQT () {
            ...
            this.tableLoading = true;
            axiosToken({
                url: '/a/chicken/chickenGroupCharacter',
                data: {
                    strain: this.searchForm.strain,
                    gen: this.searchForm.gen,
                    startTime: this.searchForm.time[0],
                ...
                let page = res.data.page;
                // 表格数据 只要需要提交的
                this.tableQT.data = page.list ? page.list.map(e => ({
                    ...rowMaker(e),
                    value: e.fieldType === 1 ? e.dubValue : e.value
                })) : [];
                // 无论是个体还是群体都会用到的设置
        ...
        save (type) {
            ...
            // 提交设置
            axiosToken({
                url: type === 'GT' ? '/a/chicken/chickenIndividualCharacter' : '/a/chicken/chickenGroupCharacter',
                method: 'delete',
                data: rows
            })
            .then(res => {
